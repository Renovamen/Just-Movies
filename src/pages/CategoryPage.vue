      ...
      <a-col :span="16">
        ...
        <ul class="movie-list">
          <router-link tag="li" :to="{ name: 'MovieDetail', params: {id: movie._id} }" class="movie-item" v-for="movie in movies" :key="movie._id">
            <movie-list-item>
              <img v-bind:src="movie.poster" alt="" class="image" slot="thumbnail">
              <div class="title" slot="title">{{ movie.title }}</div>
              <div class="info" slot="info">{{ movie.pubdate[0] }}</div>
...
<script>
  ...
  export default {
    ...
    created() {
      this.getMoviesByTag(this.tag_gen, this.tag_cou)
    },
    methods: {
      selectTag_Gen(index) {
      ...
      getMoviesByTag(tag_gen, tag_cou) {
        getMovieTag(tag_gen, tag_cou).then(res => {
          this.movies = res
        })
      }
    },
