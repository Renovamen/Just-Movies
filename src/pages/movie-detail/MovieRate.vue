  ...
  <div class="movie-rate">
    ...
    <div class="score-wrapper">
      ...
      <div class="star-comments">
        ...
        <div class="comments">
          已有{{ movie.rating.rating_people }}人评价
        </div>
      </div>
    </div>
    <ul class="rating-bar-list">
      <li class="rating-bar" v-for="(score, index) in movie.rating.stars" :key="index">
        <div class="index">{{ 5 - index }}星</div>
        <div class="bar" :style="{width: Percent(score, movie.rating.stars) + '%'}"></div>
        <div class="percent">{{ Percent(score, movie.rating.stars) }}%</div>
      </li>
    </ul>
  </div>
</template>
